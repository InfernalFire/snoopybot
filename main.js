...
var commands = [
	...
	// -> Play Song
	{
		...
		exec: function(message, params)
		{
			...
			if(_msg.includes("?list="))
			{
				message.channel.send("handling playlist... stuff - cur in progress :D :D :D :D");
				return;
			}
...
bot.on('ready', () =>
{
    ...
    bot.user.setUsername("Snoopy");
		bot.user.setGame("-help || " + bot.guilds.size + " Guilds");
		bot.user.setStatus('dnd');
	console.log('[Snoopy] Logged in...');
	handleMusic_Setup();
});

bot.login(config.tokens.discord_token);
//-------------------------->
// -> Functionality
